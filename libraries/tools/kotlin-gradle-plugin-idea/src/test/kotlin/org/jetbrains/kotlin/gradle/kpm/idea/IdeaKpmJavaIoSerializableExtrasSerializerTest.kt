...
class IdeaKpmJavaIoSerializableExtrasSerializerTest {
    ...
    fun `serialize - deserialize - non serializable object`() {
        ...
        val serializationFailureReportsCount = context.logger.reports.count { report ->
            report.message?.startsWith(IdeaKpmJavaIoSerializableExtrasSerializer.ErrorMessages.serializationFailure) ?: false
        }
    ...
    fun `serialize - deserialize - exception during deserialization`() {
        ...
        assertTrue(
            reportMessage.startsWith(IdeaKpmJavaIoSerializableExtrasSerializer.ErrorMessages.deserializationFailure),
            "Expected issue report to start with ${IdeaKpmJavaIoSerializableExtrasSerializer.ErrorMessages.deserializationFailure}"
        )
